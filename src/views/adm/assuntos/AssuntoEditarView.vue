...
onBeforeMount(async () => {
    let config = {
        method: 'get',
        url: 'https://apiminhamente.onrender.com/assunto/'+route.params.id
    };

    try {
        let { data } = await axios(config)

        nome.value = data.assunto.nome
        icone.value = data.assunto.icone
    } catch (error) {
        console.log(error);
        router.go(-1)
    }
...
async function editar(){
    ...
    if (nomeUsar != undefined && nomeUsar.trim().length != 0) {
        formData.append('nome', nomeUsar);
    }
    
    if (iconeUsar != undefined) {
        formData.append('icone', iconeUsar);
    }

    try {
        let assunto = await axios.put('https://apiminhamente.onrender.com/assunto', formData, {headers: {'authorization': `Bearer ${storeLogin.token}`}})
